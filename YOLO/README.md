# 🚴‍♂️ YOLO Training, Validation & XAI Pipeline

This directory contains the complete toolkit for processing synthetic data generated by Isaac Sim, training various **YOLO architectures (v8, v9, v10, 11, 26)**, and auditing results through visual reports and Explainable AI (XAI).
...
├── batch_auditor.py            # Automates auditing of multiple models at once
├── compare_models.py           # Interactive Model Benchmarking CLI
├── xai_YOLO.py                 # Explainable AI Suite (Spatial & Structural Analysis)
...
4. **Interactive Mode (Model Selection):** Opens a console assistant to choose the YOLO version (v8, v9, v10, 11, 26), model size (Nano, Small, Medium), and experiment name.

```bash
python train_YOLO.py
```

5. **Fine-Tuning Existing Models:** To continue training from one of your previously trained models (instead of base COCO), use the `--finetune` flag. It will open an interactive menu to select the base model. You can also specify an initial learning rate (`--lr0`).

```bash
python train_YOLO.py --finetune --lr0 0.0001
```
...
### Step 5: Explainable AI (xai_YOLO.py)

Deep dive into your models' decision-making processes using our dedicated XAI suite.

```bash
python xai_YOLO.py
```

This tool offers three main functionalities:
1. **Spatial XAI (Grad-CAM):** Extracts feature heatmaps for every intermediate layer in the network to visualize exactly where the model is looking and how features are composed.
2. **Structural XAI:** Compares a Fine-Tuned model against its Base (e.g., COCO) to analyze weight drift, illumination/activation bias shifts, and sparsity (measuring Catastrophic Forgetting).
3. **Full Suite:** Performs both Spatial and Structural analyses automatically.

### Step 6: Batch Auditing (batch_auditor.py)

Evaluate multiple models at once across synthetic datasets, real images, or videos. This tool automates the process and dynamically generates reports for each model.

```bash
python batch_auditor.py
```
*(Add `--complete` to access advanced features like video audits and forcing confidence thresholds).*
